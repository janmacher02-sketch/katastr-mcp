...
app.get("/", (_req, res) => {
  res.json({ name: "katastr-mcp", version: "1.0.0", status: "ok" });
});

app.post("/mcp", authMiddleware, async (req, res) => {
  const server = new McpServer({ name: "katastr-mcp", version: "1.0.0" });
...
app.listen(PORT, () => console.log(`katastr-mcp running on port ${PORT}`));
